...
class ForceAtlas2(var graph: GraphView) {
    ...
    private fun applyReplacement() {
        graph.vertexes().values.onEachIndexed { count, it -> it.position = nodes[count].x to nodes[count].y }
    }
    ...
    private fun initAlgo() {
        ...
        graph.vertexes().values.onEachIndexed { count, it ->
        ...
        for (edge in graph.edges()) {
            val e = Edge()
            e.node1 = nodes[findNodeWithId(edge.key.vertexes.first.element)]
            e.node2 = nodes[findNodeWithId(edge.key.vertexes.second.element)]
            edges.add(e)
        }
    }
